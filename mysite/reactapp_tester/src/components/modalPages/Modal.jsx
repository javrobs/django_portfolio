...
export default function Modal({children, action, onClose, color}) {
    ...
    const TitleBlock = () => {
        const actions = {
            ...
            "exit": {bg:"bg-rose-700", title:"Salida"},
    ...
    const ContentBlock = () => {
        switch(action){
            ...
            case "scan":
                return <p>Escanea el código QR</p>
        }
    }


    return <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" onClick={(event)=>((event.target === event.currentTarget) && onClose())}>
