...
const Layout =() => {
    ...
    <AnimatePresence>
        {!error.tag && Boolean(error.message) &&
            <motion.div ref={errorRef} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="absolute flex sm:max-w-full max-w-screen-sm gap-1 items-center top-24 self-center bg-rose-200 bg-opacity-90 backdrop-blur-sm p-3 rounded-md shadow-md">
