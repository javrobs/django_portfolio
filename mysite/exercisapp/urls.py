
from django.urls import path
from . import views
from . import login
from . import loaders
from . import workouts
from django.contrib import admin


urlpatterns = [
    path("",views.main),
    path("login/",views.main),
    path("signup/",views.main),
    path("workouts/",views.authenticated_view),
    path("create-workout/",views.authenticated_view),
    path("edit-workout/<int:workout_id>/",views.authenticated_view),
    path('today/<int:order_id>/', views.workout_today),
    path('friends/', views.authenticated_view),
    path('profile/<int:user_id>/',views.profile_view),
    path('my-profile/',views.authenticated_view),
    path('admin/', admin.site.urls),

    path("api/login/signup_user/",login.signup),
    path("api/login/login_user/",login.login_user),
    path("api/login/load_user/",login.load_user),
    path("api/login/logout_user/",login.logout_user),
    path("api/login/respond_friendship/",login.respond_friendship),
    path("api/login/request_friendship/",login.request_friendship),

    path("api/set_rep_weight/<int:order_id>/",workouts.reps_weights),
    path("api/create_workout/<int:workout_id>/",workouts.create_workout),
    path("api/create_workout/",workouts.create_workout),
    path("api/create_session/",workouts.create_session),
    path("api/end_session/",workouts.end_session),
    path("api/edit_workouts/<int:workout_id>/",workouts.edit_workouts),
    path("api/copy_workout/",workouts.copy_workout),
    
    path("api/load/home/",loaders.home),
    path("api/load/workouts/",loaders.workouts),
    path("api/load/workout_details/<int:workout_id>/",loaders.workout_details),
    path("api/load/create_workout/",loaders.create_workout),
    path("api/load/create_workout/<int:workout_id>/",loaders.create_workout),
    path("api/load/today/<int:order_id>/",loaders.reps_weights),
    path("api/load/friends/",loaders.friends),
    path("api/load/profile/<int:profile_id>/",loaders.profile),
    path("api/load/my_profile/",loaders.my_profile),
]
