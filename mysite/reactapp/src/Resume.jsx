...
const Resume = ({ref}) => {
    ...
    return <Page ref={ref}>
        ...
        <H1Title>Tools I like to use</H1Title>
        ...
        <motion.div {...fadeUpChild} className="col-span-12 flex text-lg items-center justify-center gap-2 my-2">
            Download my resume:
            {["docx","pdf"].map(each=><a href={`/static/home/documents/Resume.${each}`} key={each} target="_blank"><Button className="h-8 w-8 text-xl" icon={`filetype-${each}`}/></a>)}
        </motion.div>
    </Page>
}
